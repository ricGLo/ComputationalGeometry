...
def DFS(G, Triangles, vertice, node_colors, cfc, counter, self):
    nodos_visitados = 0 #Counter of visited nodes
    node_colors[vertice] = 'gray'

    #Colorize with white the founded node
    ...
    for v in G[vertice] :
        if node_colors[v] == 'white':
            ...
            self.play( Create( Line([gc[0], gc[1],0], [gc2[0], gc2[1],0], stroke_width = 1.4, color = GRAY )),run_time=0.5 )  #Colorize with grey the founded node
            colorize(Triangles, vertice, v, cfc, counter, self) #Colorize the vertices of triangle whose node was found
            nodos_visitados = nodos_visitados + DFS(G, Triangles, v, node_colors, cfc, counter, self)

    #Colorize with black the explored node
    ...
    return nodos_visitados #return number of founded nodes
...
class ArtGallery(Scene): 
    def construct(self):
        ...
        #####################    INPUT    ######################
        #Important: Points of poligons must be given clockwise

        #Example 1:
        #P = [ [-8, 3.38], [-4.18, 4.34], [0.8, 3.52], [-1, 0], [5.22, -0.82], [0.82, -3.94], [-4.04, -0.64], [-4.16, -4.22], [-8.86, -2.96]     ]

        #Example 2: Hair comb case (worst case) 
        ...
        self.add( Text(" Art gallery problem  ").shift(5.5*UP).scale(1.5) ) #Print tittle in animation

        Triangles = EarClipping(P, len(P), self) #Triangulate the poligon using Ear Cliping algorithm
        
        G = MakeGraph(Triangles) #Construct dual graph of the triangulation

        #Each node of the graph G must has a color: white, black or grey, This is for DFS implementation.
        #All nodes are intialized white
        colors = {} #Node colors
        for v in G:
            colors[v] = 'white'

        #Colors of each vertex of the triangles are allocated here
        #Every vertex is initialized white
        colors_for_counting = []
        for j in range(len(Triangles)):
            c = [WHITE, WHITE, WHITE]
            colors_for_counting.append(c)

        #Counter of each color (red, blue and green) in animation
        ...
        DFS(G, Triangles, 0, colors, colors_for_counting, counter, self) #Run Depth first search

        #Find OUTPUT. Id est the number of colored vertices by the least used color 
